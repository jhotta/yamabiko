...
traced_app = TraceMiddleware(app, tracer, service="my-flask-ask-app")

@ask.launch
def launch():
    statsd.increment('yamabiki.launch')
...
def metric_is(metric):
    statsd.increment('yamabiki.whatsthevalueintent')
    card_title = render_template('card_title')
    now = int(time.time())
...
@ask.session_ended
def session_ended():
    statsd.increment('yamabiki.sessionended200')
    return "", 200
